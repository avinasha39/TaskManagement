...
export class ProjectDetailComponent implements OnInit {
  ...
  OnFormAdd(): void {
    ...
    this.dataProvider.addTask(this.id,this.AddForm.value).subscribe(res => console.log(res));
  ...
  onProjectEdit(): void {
    this.dataProvider.updateProjectDetails(this.id,this.checkoutForm.value).subscribe(res => console.log(res));
  ...
  onDelete(): void {
    this.dataProvider.deleteProjectWithId(this.id).subscribe(projResp => console.log(projResp.message) );
  ...
  getProject(id: number): void {
    ...
    console.log(this.project.value.photo);
  }
